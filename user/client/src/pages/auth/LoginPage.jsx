...
import { Loader, Eye, EyeOff, User, Lock, Linkedin } from "lucide-react"; // Linkedin kept in import in case re-enabled later
import { Link, useNavigate } from "react-router-dom";
import Login from "/login/4.png";
...
const LoginPage = () => {
  ...
  const { mutate: loginMutation, isLoading } = useMutation({
    ...
    onSuccess: async () => {
      ...
      try {
        // Fetch auth user to determine role first
        const authRes = await axiosInstance.get('/auth/me');
        const role = authRes?.data?.role?.toLowerCase?.();

        // True admins/superadmins go to admin dashboard ("/dashboard")
        if (role === 'admin' || role === 'superadmin') {
          navigate('/dashboard', { replace: true });
          return;
        }

        // For non-admins, check effective hierarchy for subadmin access
        const { data } = await axiosInstance.get('/links/hierarchy/my-hierarchy');
        const hierarchy = data?.adminHierarchy?.toLowerCase?.() || 'alumni';
        const elevated = !['alumni', 'student', 'employee'].includes(hierarchy);
        if (elevated) {
          navigate('/subadmin/dashboard', { replace: true });
          return;
        }
      } catch (e) {
        // On any error, fall back to regular home
      }
        ...
        <div className="flex items-center justify-center h-[95vh] p-2 my-4 lg:ps-2 rounded-bl-[110px] rounded-tl-[10px] rounded-tr-[10px] rounded-br-[10px] shadow-2xl bg-white/80 backdrop-blur-xl">
          <div className="flex flex-col lg:flex-row w-full h-full max-w-4xl overflow-hidden">
            ...
            <motion.div
              ...
              className="w-full lg:w-1/2 h-full flex items-center justify-center p-6 lg:p-8 bg-opacity-50 backdrop-filter backdrop-blur-xl relative"
            >
              <div className="w-full">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="w-full py-2 lg:py-3 px-4 bg-[#fe6019] text-white font-bold rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#fe6019] transition duration-200"
                    type="submit"
                    disabled={isLoading}
                  >
                    {isLoading ? (
                ...
                {/* LinkedIn OAuth button temporarily disabled as requested */}
