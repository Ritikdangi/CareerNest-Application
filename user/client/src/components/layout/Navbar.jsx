...
const Navbar = () => {
	...
	const { mutate: logout } = useMutation({
		...
		onSuccess: () => {
			// Clear all cached queries and force navigation to login
				...
				<div className='flex justify-between items-center py-3'>
					<div className='flex items-center space-x-4'>
						...
						<Link to='/' className="transform transition-transform duration-300 hover:scale-105" onClick={handleNavClick}>
							<img className='h-8 sm:h-9 md:h-11 rounded hover:animate-none' src='/careernest-logo.svg' alt='CareerNest' style={{ maxHeight: 44 }} />
						</Link>
					</div>
