...
import { getCookieOptions, getCookieName } from "../lib/cookies.js";
...
export const signup = async (req, res) => {
	try {
		...
		let headline = "CareerNest User"; // Default headline
		if (role === "admin") {
			headline = "CareerNest Admin";
		...
		const user = new User({	
			...
			// Auto-verify accounts during development or when explicitly enabled.
			isVerified: (process.env.AUTO_VERIFY_ON_SIGNUP === 'true') || process.env.NODE_ENV !== 'production'
		});
...
export const linkedInCallback = async (req, res) => {
	try {
		...
		// Step 6: Set cookie (unified options)
		const cookieName = getCookieName();
		const baseCookieOptions = getCookieOptions();
		res.cookie(cookieName, token, {
			...baseCookieOptions,
