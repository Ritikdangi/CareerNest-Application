...
export const acceptLinkRequest = async (req, res) => {
    try {
        ...
        // Allow elevated hierarchies (hod/faculty/management) to approve even if not direct recipient
        const elevatedHierarchies = [
            'hod','school_hod','faculty','school_faculty','institute_management','school_management','corporate_management'
        ];
        const isElevated = req.user.adminHierarchy && elevatedHierarchies.includes(String(req.user.adminHierarchy).toLowerCase());

        if (request.recipient._id.toString() !== targetAdminId.toString() && !isElevated) {
            console.log("  Authorization blocked - recipient mismatch and user not elevated", {
                requestRecipient: request.recipient._id.toString(),
                actingUser: targetAdminId.toString(),
                userHierarchy: req.user.adminHierarchy
            });
        ...
        // Link pairing logic:
        // If elevated user acted on someone else's request, link sender with acting user in addition to original recipient (expanded network)
        const recipientToLink = isElevated ? userId : targetAdminId;
        await User.findByIdAndUpdate(request.sender._id, { $addToSet: { Links: recipientToLink } });
        await User.findByIdAndUpdate(recipientToLink, { $addToSet: { Links: request.sender._id } });
        if (isElevated && request.recipient._id.toString() !== userId.toString()) {
            // Preserve original intended connection if missing
            await User.findByIdAndUpdate(request.sender._id, { $addToSet: { Links: request.recipient._id } });
            await User.findByIdAndUpdate(request.recipient._id, { $addToSet: { Links: request.sender._id } });
        }
...
export const rejectLinkRequest = async (req, res) => {
    try {
        ...
        if (request.recipient.toString() !== targetAdminId.toString() && !isElevated) {
            console.log("  Authorization block (reject) - recipient mismatch and user not elevated", {
                requestRecipient: request.recipient.toString(),
                actingUser: targetAdminId.toString(),
                userHierarchy: req.user.adminHierarchy
            });
...
export const approveHierarchyUpgrade = async (req, res) => {
    try {
        ...
        if (approved) {
            // Update the request's effective hierarchy
            linkRequest.adminHierarchy = newHierarchy;
            await linkRequest.save();

            // Persist the hierarchy on the user document as well so
            // middlewares using req.user and /auth/me reflect it immediately.
            try {
                await User.findByIdAndUpdate(linkRequest.sender._id, {
                    $set: {
                        adminHierarchy: newHierarchy
                    }
                }, { new: true });
            } catch (persistErr) {
                console.warn('⚠️ Failed to persist adminHierarchy on User; continuing:', persistErr?.message || persistErr);
            }
...
export const getSubAdminPendingRequests = async (req, res) => {
    try {
        const currentUserId = req.user?._id;
        const currentHierarchy = req.user?.adminHierarchy;
        ...
        // Build base query. For elevated hierarchies (hod/faculty/management variants)
        // widen visibility to all pending requests so they can assist in onboarding.
        const elevatedHierarchies = [
            'hod','school_hod','faculty','school_faculty','institute_management','school_management','corporate_management'
        ];

        const isElevated = currentHierarchy && elevatedHierarchies.includes(String(currentHierarchy).toLowerCase());

        const baseQuery = { status: 'pending' };
        if (!isElevated) {
            // Non‑elevated subadmin: restrict to requests addressed to them
            baseQuery.recipient = targetAdminId;
        }

        console.log('🔎 SubAdmin pending requests query:', { isElevated, currentHierarchy, baseQuery });

        const pendingRequests = await LinkRequest.find(baseQuery)
            .select('sender recipient rollNumber batch courseName selectedCourse status createdAt updatedAt adminHierarchy requestedHierarchy hierarchyChangeReason hierarchyStatus')
        ...
        res.status(200).json({
            ...
            networkInfo: isElevated ? {
                adminId: currentUserId,
                scope: 'global-pending',
                totalPendingVisible: totalCount,
                message: 'Elevated role: showing all pending join requests'
            } : {
