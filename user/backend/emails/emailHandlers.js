...
export const sendWelcomeEmail = async (email, name, profileUrl) => {
	...
	try {
		const response = await mailtrapClient.send({
			from: sender,
			to: recipient,
			subject: "Welcome to CareerNest – Your Alumni Community Awaits!",
...
export const sendCommentNotificationEmail = async (
	...
	try {
		const response = await mailtrapClient.send({
			from: sender,
			to: recipient,
			subject: "Someone commented on your post in CareerNest!",
...
export const sendLikeNotificationEmail = async (recipientEmail, recipientName, likerName, postUrl, postContent) => {
	...
	try {
		const response = await mailtrapClient.send({
			from: sender,
			to: recipient,
			subject: "Your post received a new reaction on CareerNest!",
...
export const sendReplyNotificationEmail = async (recipientEmail, recipientName, replierName, postUrl, replyContent) => {
	...
	try {
		const response = await mailtrapClient.send({
			from: sender,
			to: recipient,
			subject: "Someone replied to your comment on CareerNest!",
...
export const sendMentionNotificationEmail = async (recipientEmail, recipientName, mentionerName, postUrl, postContent) => {
	...
	try {
		const response = await mailtrapClient.send({
			from: sender,
			to: recipient,
			subject: "You were mentioned in a CareerNest post!",
...
export const sendPostStatusNotificationEmail = async (recipientEmail, recipientName, reviewerName, postStatus, postUrl, postContent, feedback) => {
	const recipient = [{ email: recipientEmail }];
	const subject = postStatus === 'approved' ? "Your CareerNest post is now live!" : "Your CareerNest post was not approved";
